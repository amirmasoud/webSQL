...
function w3WebSQLInit() {
	var w3DBObj = this;
	w3Database = window.openDatabase('ChkoshSQLdemoDB', '1.0', 'ChkoshSQLdemoDB', 2 * 1024 * 1024);
				...
				tx.executeSql(sql,[],function (tx, results)
					{
						...
						if (len > 0) {
							txt = "";
							txt = txt + "<p>Record count: " + len + "</p>";
							txt = txt + "<table class='table table-striped'><tr>";
							...
							if (DBChanges === true) {
								txt = "<p>you changed database sets.";
								if (results.rowsAffected > 0) {txt = txt + " Modified record: " + results.rowsAffected; }
								resultContainer.innerHTML = txt + "</p>";
							} else {
								txt = "<p>without result.</p>";
		...
		w3Database.transaction(function (tx)
			{
				...
				function w3DBInfo() {
					...
					txt = txt + "<th>table name</th>";
					txt = txt + "<th>records</th>";  						
					txt = txt + "</tr>";
					for (i = 0; i < tblnames.length; i++) {
						txt = txt + "<tr>";	   
						txt = txt + "<td title='click for the content of table " + tblnames[i] + "' onclick='w3schoolsWebSQL1.selectStar(\"" + tblnames[i] + "\")'>" + tblnames[i] + "</td>";  
				...
				function w3DBViewInfo() {
					...
					txt = txt + "<h3>VIEW:</h3>";
					txt = txt + "<table class='table'><tr>";
					txt = txt + "<th>view name</th>";  
					txt = txt + "<th>record</th>";
					txt = txt + "</tr>";
					for (i = 0; i < viewnames.length; i++) {
						txt = txt + "<tr>";	   
						txt = txt + "<td title='click for the view  " + viewnames[i] + "' onclick='w3schoolsWebSQL1.selectStar(\"" + viewnames[i] + "\")'>" + viewnames[i] + "</td>";  
						txt = txt + "<td>" + viewrecordcounts[i] + "</td>";  				
						txt = txt + "</tr>";	   
					}
				...
				function w3DBIndexInfo() {
					...
					txt = txt + "<h3>INDEX:</h3>";
					txt = txt + "<table class='table'><tr>";
					txt = txt + "<th>name of the index</th>";  
					txt = txt + "</tr>";
					for (i = 0; i < viewnames.length; i++) {
						txt = txt + "<tr>";	   
						txt = txt + "<td>" + indexnames[i] + "</td>";  
						txt = txt + "</tr>";	   
					}
	...
	this.w3ClearDatabase = function() {
		var warn = window.confirm("restore database?");
		if (warn === false) {
			return false;
		}
	...
	this.w3InitCustomers = function() {
		w3Database.transaction(function (tx)
			...
			,function (err) {
				if (err.message.indexOf("Customers هم اکنون هم وجود دارد") === -1) {
	...
	this.w3InitEmployees = function() {
		w3Database.transaction(function (tx)
			...
			,function (err) {
				if (err.message.indexOf("Employees هم اکنون هم وجود دارد") === -1) {
	...
	this.w3InitOrderDetails = function() {
		w3Database.transaction(function (tx)
			...
			,function (err) {
				if (err.message.indexOf("OrderDetails هم اکنون هم وجود دارد") === -1) {
	...
	this.w3InitOrders = function() {
		w3Database.transaction(function (tx)
			...
			,function (err) {
				if (err.message.indexOf("Orders  هم اکنون هم وجود دارد") === -1) {
	...
	this.w3InitProducts = function() {
		w3Database.transaction(function (tx)
			...
			,function (err) {
				if (err.message.indexOf("Products  هم اکنون هم وجود دارد") === -1) {
	...
	this.w3InitShippers = function() {
		w3Database.transaction(function (tx)
			...
			,function (err) {
				if (err.message.indexOf("Shippers  هم اکنون هم وجود دارد") === -1) {
	...
	this.w3InitSuppliers = function(n) {
		w3Database.transaction(function (tx)
			...
			,function (err) {
				if (err.message.indexOf("Suppliers") === -1) {
